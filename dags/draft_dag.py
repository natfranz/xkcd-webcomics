...
def fetch_xkcd_data_loop(**kwargs):
    ti = kwargs['ti']
    last_value = ti.xcom_pull(task_ids='find_last_value')
    max_value = ti.xcom_pull(task_ids='find_max_value')
    df = pd.DataFrame()
...
def process_data(**kwargs):
    ti = kwargs['ti']
    df = ti.xcom_pull(task_ids='fetch_xkcd_data')
    df = df.assign(
...
def load_to_db(**kwargs):
    ...
    # Assuming you have a function to load DataFrame to a database
    # Replace this with your actual function or code to load data to DB
    load_data_to_db(df)
...
fetch_xkcd_data = PythonOperator(
...
branching >> fetch_xkcd_data

fetch_xkcd_data >> process_data_task >> load_to_db_task >> end_pipeline
